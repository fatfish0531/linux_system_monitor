...
monitor_cpu() { 
    print_banner 
    echo "【模块：CPU 监控】" 
    echo "功能说明：监控系统 CPU 使用率、运行队列长度、上下文切换次数" 
    echo " "
    ...
    print_table_line "${cpu_widths[@]}" 
    echo "CPU 监控完成。采样次数：$COUNT, 间隔：${INTERVAL}秒" 
}
...
monitor_disk() { 
    print_banner 
    echo "【模块：磁盘 I/O 监控】" 
    echo "功能说明：监控磁盘 I/O 吞吐量、IOPS、I/O 延迟、队列深度" 
    echo " "
...
for dev in $disk_devices; do
    ...
    print_table_line "${disk_widths[@]}" 
    echo "磁盘 I/O 监控完成。采样次数：$COUNT, 间隔：${INTERVAL}秒" 
}
 
monitor_network() { 
    print_banner 
    echo "【模块：网络监控】" 
    echo "功能说明：监控网络接口带宽使用、TCP/UDP 连接统计、按进程网络使用" 
    echo " " 
    echo "━━━ 网络接口带宽监控 ━━━"
...
monitor_process() { 
    ...
    ps aux --no-headers 2>/dev/null | sort -rn -k 3 | head -10 | while read line; do 
    ...
    ps aux --no-headers 2>/dev/null | sort -rn -k 4 | head -10 | while read line; do 
        local pid=$(echo $line | awk '{print $2}') 
        local user=$(echo $line | awk '{print $1}') 
        local mem=$(echo $line | awk '{print $4}') 
    ...
    ps -eLf --no-headers 2>/dev/null | awk '{print $2, $NF}' | sort | uniq -c | sort -rn | head -10 | while read count pid comm; do 
...
monitor_slab() { 
    ...
    grep -E "^(dentry|inode_|kmalloc-|vm_area_struct)" /proc/slabinfo 2>/dev/null | while read line; do 
...
monitor_once() { 
    ...
    echo "━━━ 系统信息 ━━━" 
    echo "主机名: $(hostname)" 
    echo "操作系统：$(grep PRETTY_NAME /etc/os-release 2>/dev/null | sed 's/PRETTY_NAME="//;s/"$//')" || echo "操作系统：Unknown"
    echo "内核版本: $(uname -r)" 
    echo "架构: $(uname -m)" 
    ...
    df -h 2>/dev/null | grep -vE "^(Filesystem|文件系统)" | head -10 | while read filesystem size used avail pct mount; do 
    ...
    ps aux --no-headers 2>/dev/null | sort -rn -k 3 | head -5 | while read line; do 
        local pid=$(echo $line | awk '{print $2}') 
        local user=$(echo $line | awk '{print $1}') 
        local cpu=$(echo $line | awk '{print $3}') 
        local mem=$(echo $line | awk '{print $4}') 
    ...
    ps aux --no-headers 2>/dev/null | sort -rn -k 4 | head -5 | while read line; do 
        local pid=$(echo $line | awk '{print $2}') 
        local user=$(echo $line | awk '{print $1}') 
        local mem=$(echo $line | awk '{print $4}') 
        local rss=$(echo $line | awk '{print $6}') 
